...
dataset_mouse = 'MT062';
dataset_date = '_20190816';
dataset = strcat(dataset_mouse, dataset_date);

load(strcat('wpli_',dataset,'_03_MWC_PosNeg_2sPre4sPostStimRaster_laser3_compil20200618_coeff.mat'));
load(strcat('wpli_',dataset,'_03_MWC_PosNeg_2sPre4sPostStimRaster_laser3_compil20200618_subset.mat'));
load(strcat(dataset,'_FRA_OptoStim_03.mat'));
load(strcat(dataset,'_RedCells_SlopeRemoval_Movie.mat'));
load(strcat('F_',dataset,dataset_mouse,'_tifStacks_plane1_proc.mat'));

%dir = strcat('/home/janaki/Dropbox/project_with_melanie/DataForJanaki/',dataset);
%cd(dir); 

%% Recover cells from the graph edges

ncells = 1/2*(1+sqrt(1+8*size(subset,2)));
ind_track = zeros(size(subset,2),2);
...
%% find cells neighbouring to red cells

% check which red cells respond to light only (no sound), have to respond
% to both light conditions
responsiveRedcells = [];
responsiveRedcellstoBothLight = [];
for i = 1:length(Ind_cells_red)
    if any(abs(Cells_ActivityChange2([2,3],Ind_cells_red(i))))
        ...
        responsiveRedcellstoBothLight = [responsiveRedcellstoBothLight, ...
            Ind_cells_red(i)*all(abs(Cells_ActivityChange2([2,3],Ind_cells_red(i))))];
    end
end
...
%% plotting cells from nnmf

%close all;
for ii = 1:size(subset,1)
    contributingSubset = subset(ii,subset(ii,:)>0)';
    contributingEdges = ind_track(subset(ii,:)>0,:);    
    subsetQuantiles = quantile(contributingSubset,0.995);
    quantileEdges = contributingEdges(contributingSubset>=subsetQuantiles(length(subsetQuantiles)),:);
    ...
    for ii = 1:length(Allcells_ROI)
        h = patch(spatialInfo.ROIs{ii}(:,2),spatialInfo.ROIs{ii}(:,1),'y',...
            'facealpha',0.8,'edgealpha',0,'LineWidth',1,'facecolor','r');
    end
     ...
     for ii = 1:length(responsiveRedcells)
        h = patch(spatialInfo.ROIs{responsiveRedcells(ii)}(:,2),spatialInfo.ROIs{responsiveRedcells(ii)}(:,1),'y',...
            'facealpha',0.8,'edgealpha',0,'LineWidth',1,'facecolor','g');
    end
    %}
    for kk = 1:length(quantileEdges)            
        plot([Allcells_ROI(quantileEdges(kk,1),2), Allcells_ROI(quantileEdges(kk,2),2)], ...
            [Allcells_ROI(quantileEdges(kk,1),1), Allcells_ROI(quantileEdges(kk,2),1)],'b');
    end
end

%% why are there horizontal connections?
...
for ii=1:length(quantileEdges)
    figure; hold on;
    for jj=[3,6,9,12,15,18,21]
        plot(xcorr(mean_Trace(jj,:,quantileEdges(ii,1)), mean_Trace(jj,:,quantileEdges(ii,2)), 'coeff'));
    end
end
%}

...
if mouseline == 'VIP'
    cd '/home/janaki/Dropbox/project_with_melanie/DataForJanaki/networkFiles/VIP_NMFDatasets/'
elseif mouseline == 'SOM'
    cd '/home/janaki/Dropbox/project_with_melanie/DataForJanaki/networkFiles/SOM_NMFDatasets/'
end
...
if mouseline == 'VIP'
    folderpath=['/home/janaki/Dropbox/project_with_melanie/DataForJanaki/networkFiles/VIP_connectivityData'];
elseif mouseline == 'SOM'
    folderpath=['/home/janaki/Dropbox/project_with_melanie/DataForJanaki/networkFiles/SOM_connectivityData'];
end
cd(folderpath)
...
if mouseline == 'VIP'
    folderpath=['/home/janaki/Dropbox/project_with_melanie/DataForJanaki/networkFiles/VIP_ActivityData'];
elseif mouseline == 'SOM'
    folderpath=['/home/janaki/Dropbox/project_with_melanie/DataForJanaki/networkFiles/SOM_ActivityData'];
end
cd(folderpath)
...
if is_PosNeg ==1
    for k=laserconditions
        coeff{k,1}.coeff_all=coeff{k,1}.coeff./255;
        coeff{k,1}.coeff_pos=coeff{k,1}.coeff_all(:,1:end/2);
        coeff{k,1}.coeff_neg=coeff{k,1}.coeff_all(:,end/2+1:end);
        coeff{k,1}.coeff=coeff{k,1}.coeff_pos-coeff{k,1}.coeff_neg;    

        coeff{k,1}.mean_relativecoeff=mean(coeff{k,1}.coeff,2);
        for i=1:size(coeff{k,1}.coeff,1)
            coeff{k,1}.entropy(i)=wentropy(coeff{k,1}.coeff(i,:),'shannon');
        end
        connectivity_data{k,1}.data_all=double(connectivity_data{k,1}.data)./255;
        connectivity_data{k,1}.data_pos=connectivity_data{k,1}.data_all(:,1:end/2);
        connectivity_data{k,1}.data_neg=connectivity_data{k,1}.data_all(:,end/2+1:end);
        connectivity_data{k,1}.data=connectivity_data{k,1}.data_pos-connectivity_data{k,1}.data_neg; 
    end
end
...
figure(oldFigCnt); oldFigCnt = figcntFn(oldFigCnt);
...
for k=laserconditions % one figure per laser condition
    ...
    for i=1:size(subset{k}.subset,1)
        ...
        line([-3 -3],[9.5 10.5],'Color','r','LineWidth',3) % red cells
        % x axis
        ...
        line([9.5 10.5],[-3 -3],'Color','r','LineWidth',3) % red cells      
        
        title({[mouseline,' - ',mouse,' - ',date,'-',recnumb], ['matrix of subset ',num2str(i),', laser power',num2str(k)]})
        xlabel('nodes')
        ylabel('nodes')
        axis tight
        set(gca,'YDir','reverse') 
...
for i=laserconditions % one figure per laser condition
    ...
    for j=1:size(mean_coeff{i},3)
        ...
        title({[mouseline,' - ',mouse,' - ',date,'-',recnumb], ['subset ',num2str(j),', laser power',num2str(i)]})
        xlabel('Time (s)')
        ylabel('Coeff')
        xlim([0, 181/fr])
    end
    ...
    for j=1:size(coeff_rastord{k},3) % number of subsets per laser condition
        for i=1:length(stimInfo.intensity) % sound conditions           
            ...
            for l=1:length(Tdelay_test)
                [coeff_tstats{k}.tstats_time(i,j,l), coeff_tstats{k}.pstats_time(i,j,l)]=ttest(nanmean(coeff_rastord{k}(repeats*(i-1)+1:repeats*i,1:round(fr*Tdelay_windur),j),2),...
                    nanmean(coeff_rastord{k}(repeats*(i-1)+1:repeats*i,round(fr*(preOnsetTime+Tdelay_test(l))):round(fr*(preOnsetTime+Tdelay_test(l)+Tdelay_windur)),j),2),'Alpha',alpha);
            ...
            coeff_tstats{k}.Tdelay(i,j)=min(Tdelay_test(coeff_tstats{k}.pstats_time(i,j,:)==min(coeff_tstats{k}.pstats_time(i,j,:))));
           
            % compute stats, average and std with chosen Tdelay
            [coeff_tstats{k}.tstats(i,j), coeff_tstats{k}.pstats(i,j)]=ttest(nanmean(coeff_rastord{k}(repeats*(i-1)+1:repeats*i,1:round(fr*Tdelay_windur),j),2),...
                nanmean(coeff_rastord{k}(repeats*(i-1)+1:repeats*i,round(fr*(preOnsetTime+coeff_tstats{k}.Tdelay(i,j))):round(fr*(preOnsetTime+coeff_tstats{k}.Tdelay(i,j)+Tdelay_windur)),j),2),'Alpha',alpha);
...
% plot the time-varying pstats i.e. how does pstat for each sound condition
% vary with tau_delay. 
...
for i=laserconditions % one figure per laser condition
    ...
    for j=1:size(mean_coeff{i},3) % number of subsets
        ...
        title({[mouseline,' - ',num2str(subset_filename(8:11)),'-',num2str(subset_filename(12:19))], ['subset ',num2str(j),', laser power',num2str(i)]})
        xlabel('frame')
        ylabel('p-stat')
...
for k=laserconditions % laser conditions
    ...
    for j=1:size(coeff_rastord{k},3) % number of subsets per laser condition
        ...
        coeff_tstats{k}.Tdelay_ave(j)=min(Tdelay_test(coeff_tstats{k}.pstats_ave_time(j,:)==min(coeff_tstats{k}.pstats_ave_time(j,:))));
...
for i=laserconditions % one subplot per laser condition
    for j=1:size(mean_coeff{i},3)
        ...
        title({[mouseline,' - ',num2str(subset_filename(8:11)),'-',num2str(subset_filename(12:19))], ['laser power',num2str(i)]})
        xlabel('Time (s)')
        ylabel('MEAN Coeff')
        xlim([0, 181/fr])
    end
...
for k=laserconditions
    ...
    title({[mouseline,' - ',mouse,' - ',date,'-',recnumb], ['laser power ',num2str(k)],'Entropy over mean trace'})
    subplot(3,3,3+k),
...
nedges=size(subset{laserconditions(1)}.subset,2);
nsubsets = zeros(length(laserconditions),1);
...
loglog(thres_weight,nedgesperthres_allsubsperlaser,'LineWidth',2)
        title({[mouseline,' - ',mouse,' - ',date,'-',recnumb], 'Total number of edges above threshold', '(across subsets) per laser condition'})
...
semilogx(thres_weight,nnodesperthres_allsubsperlaser,'LineWidth',2)
        title({[mouseline,' - ',mouse,' - ',date,'-',recnumb], 'Total number of nodes above threshold', '(across subsets) per laser condition'})
xlabel('threshold / nedges')
...
loglog(thres_weight,nedgesperthres_allsubsperlaser./(nedges*nsubsets)','LineWidth',2)
        title({[mouseline,' - ',mouse,' - ',date,'-',recnumb], 'Ratio of edges above threshold', '(across subsets) per laser condition'})
xlabel('threshold / nedges')
ylabel('Ratio of edges above threshold')
legend('laser 1', 'laser 2', 'laser 3')
subplot(224),
semilogx(thres_weight,nnodesperthres_allsubsperlaser/ncells,'LineWidth',2)
        title({[mouseline,' - ',mouse,' - ',date,'-',recnumb], 'Ratio of nodes above threshold', '(across subsets) per laser condition'})
xlabel('threshold / nedges')
...
for k=laserconditions
    ...
    title({[mouseline,' - ',mouse,' - ',date,'-',recnumb], ['laser ',num2str(k)], 'Total number of edges above threshold'})
    xlabel('threshold / nedges')
    ylabel('Number of edges above threshold')
    subplot(4,3,3+k),
    ...
    title({'Binary clustering coefficient','per subset, mean over all nodes'})
    xlabel('threshold / nedges')
    ...
    title({'Weighted clustering coefficient','per subset, mean over all nodes'})
    xlabel('threshold / nedges')
    ylabel('Weighted clustering coeff')
end

%% save a few figures: go to folder (this should be moved to the respective subsections)
...
b=[];
for i=laserconditions
   a=[a, std(subset{i}.subset')]; %#ok<*AGROW>
...
for i=laserconditions % laser conditions
    ...
    for j=1:size(subset{i}.subset,1) % number of subsets        
        % threshold with std over all subsets all laser conditions (std(all edges))
...
% demanding to do that to get rid of all nodes that are not involved in subset?
...
for i=laserconditions % laser conditions
    ...
    for j=1:size(subset{i}.subset_thres,1) % subset j in laser condition i
        subset{i}.subset_thresnodes{j}=unique(ind_track(subset{i}.subset_thres{j}(:),:)); %subset.subset_thres contains edges, so converting to nodes
        ...
        if length(subset{i}.subset_thres{j}) >1 % if thresholded subset is not empty or there is more than one edge (one edge = 2 nodes = no triangles for clustering)
            ...
            % right now : funct connectivity matrix with just the relevant nodes, but could maybe
            % be replaced with matrix with plenty of zeros for further readability if I want to save these matrices later
            % this matrix has the time traces of the correlation between thresholded nodes
            tic
            for l=laserconditions % laser conditions
                subset{i}.FC_matrix{j,l}=zeros(length(subset{i}.subset_thresnodes{j}),length(subset{i}.subset_thresnodes{j}),size(connectivity_data{l}.data,2));
                for k=1:length(subset{i}.subset_thres{j})
                    ...
                    subset{i}.FC_matrix{j,l}(ind1,ind2,:)=connectivity_data{l}.data(subset{i}.subset_thres{j}(k),:);
                    subset{i}.FC_matrix{j,l}(ind2,ind1,:)=connectivity_data{l}.data(subset{i}.subset_thres{j}(k),:);
                end
                % now that FC matrix is filled up, calculate clustering time point
                % per time point

                for m=1:size(subset{i}.FC_matrix{j,l},3) % total number of frames in that laser condition
                    clustering{i}.clustering{j,l}(:,m)=clustering_coef_wu(subset{i}.FC_matrix{j,l}(:,:,m));
                end

                % create raster
                for jj=1:size(coeff{i}.coeff,2)/181 %number of trials per laser condition
...
% one figure per laser condition can be laser conditions x thresholded subsets of the given laser condition and have the
...
for i=laserconditions % laser conditions
    ...
    for j=1:size(subset{i}.subset,1) % number of subsets
        for l=laserconditions % number of laser conditions
            if size(clustering{i}.mean_clust{j,l})
                ...
                title({[mouseline,' - ',num2str(subset_filename(8:11)),'-',num2str(subset_filename(12:19))], ...
    ...
    sgtitle('Average clustering over nodes');
end
...
for i=laserconditions % laser conditions
    figure(oldFigCnt); oldFigCnt = figcntFn(oldFigCnt);
    tiledlayout(size(subset{i}.subset,1),3);
    for j=1:size(subset{i}.subset,1) % number of subsets
        for l=laserconditions % number of laser conditions
            nexttile;
            if size(clustering{i}.mean_clust{j,l})  
    ...
    sgtitle('Average clustering over nodes and sound amplitudes');
end

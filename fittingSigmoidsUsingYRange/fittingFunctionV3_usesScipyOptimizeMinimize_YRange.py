...
def fittingStatistics_v3(responses_all, 
                         sem_all,
                         amplitude,
    ...
    for i in range(numSoundResponsiveCells):
        fitParamsSigmoidNoLight = scipy.optimize.minimize(sigmoid_fn, [0.4,-1.4,11,10],
        ...
        fitParamsGaussNoLight = scipy.optimize.minimize(gaussian_fn, [-0.4,60,15,0],
                                                args=(response_noLight[:,i],sem_noLight[:,i]),
        ...
        fitParamsSigmoidMidLight = scipy.optimize.minimize(sigmoid_fn, [0.4,-1.4,11,10],
        ...
        fitParamsGaussMidLight = scipy.optimize.minimize(gaussian_fn, [-0.4,60,15,0],
                                                    args=(response_midLight[:,i],sem_midLight[:,i]),
                                                    method = 'Powell', bounds = gaussBnds,
                                                    options={'maxiter':30000, 'maxfev':30000,'ftol':0.01, 'disp':0}).x
        ...
        fitParamsSigmoidHighLight = scipy.optimize.minimize(sigmoid_fn, [0.4,-1.4,11,10],
        ...
        fitParamsGaussHighLight = scipy.optimize.minimize(gaussian_fn, [-0.4,60,15,0],
                                                    args=(response_highLight[:,i],sem_highLight[:,i]),
                                                    method = 'Powell', bounds = gaussBnds,
                                                    options={'maxiter':30000, 'maxfev':30000,'ftol':0.01, 'disp':0}).x

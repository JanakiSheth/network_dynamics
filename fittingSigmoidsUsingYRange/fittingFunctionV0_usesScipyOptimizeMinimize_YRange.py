...
def fittingStatistics_v0(responses_all, 
                         sem_all,
                         amplitude,
    ...
    """
    sigBnds = [(-np.inf,np.inf),(-np.inf,np.inf),(-np.inf,np.inf),(-np.inf,-5)]
    ...
    for i in range(numSoundResponsiveCells):
        ...
        """
        Fitting procedure using scipy.optimize.minimize (method = neadler-mead, 
        bounds = (-inf,-inf,-inf,0.5),(inf,inf,inf,inf)). 

        To do: take different starting points and take union
        """
        
        fitParamsSigmoidNoLight = scipy.optimize.minimize(sigmoid_fn, [0.4,-0.8,11,-5],
        ...
        fitParamsGaussNoLight = scipy.optimize.minimize(gaussian_fn, [-0.8,89,7,0],
                                                     args=(response_noLight[:,i],sem_noLight[:,i]),
        ...
        fitParamsSigmoidMidLight = scipy.optimize.minimize(sigmoid_fn, [0.4,-0.8,11,-5],
        ...
        fitParamsGaussMidLight = scipy.optimize.minimize(gaussian_fn, [-0.8,89,7,0],
                                                    args=(response_midLight[:,i],sem_midLight[:,i]),
                                                    method = 'Powell', bounds = gaussBnds,
                                                    options={'maxiter':30000, 'maxfev':30000,'ftol':0.01, 'disp':0}).x
        ...
        fitParamsSigmoidHighLight = scipy.optimize.minimize(sigmoid_fn, [0.4,-0.8,11,-5],
        ...
        fitParamsGaussHighLight = scipy.optimize.minimize(gaussian_fn, [-0.8,89,7,0],
                                                    args=(response_highLight[:,i],sem_highLight[:,i]),
                                                    method = 'Powell', bounds = gaussBnds,
                                                    options={'maxiter':30000, 'maxfev':30000,'ftol':0.01, 'disp':0}).x
    ...
    """
    Nonmonotonic if MI  < 0.3, Monotonic if MI >= 0.7, maybe either if MI >=0.3 and MI < 0.7
    """
    ...
    noLightGaussLowRSquared.append(noLightGaussComplete)
    noLightGaussLowRSquaredFitParams.append(fitParamsGaussNoLight_allCells[:,noLightGaussComplete])
    noLightGaussLowRSquaredValues.append(rSquaredGaussNoLight_allCells[noLightGaussComplete])
    
    """
    for j in noLightGaussComplete: 
        plt.figure()
        ...
        plt.show()
        print(j, miNoLight_allCells[j], rSquaredGaussNoLight_allCells[j], fitParamsGaussNoLight_allCells[:,j])
    """    
    for j in noLightSigmoidComplete:
        plt.figure()
        [y0, yrange, x0, delta_x] = fitParamsSigmoidNoLight_allCells[:,j]
        y = y0 + yrange/(1+np.exp((x0-amplitude)/delta_x)); 
        plt.plot(amplitude, response_noLight[:,j],'k')
        plt.plot(amplitude, y,'k--') 
        plt.show()
        print(j, miNoLight_allCells[j], rSquaredSigmoidNoLight_allCells[j], fitParamsSigmoidNoLight_allCells[:,j])    
    ...
    midLightGaussLowRSquared.append(midLightGaussComplete)
    midLightGaussLowRSquaredFitParams.append(fitParamsGaussMidLight_allCells[:,midLightGaussComplete])
    midLightGaussLowRSquaredValues.append(rSquaredGaussMidLight_allCells[midLightGaussComplete])
    ...
    for j in GaussianToKeep:        
        ...
        plt.plot(amplitude, y,'k--') 
        plt.show()
        print(j, miMidLight_allCells[j], rSquaredGaussMidLight_allCells[j], fitParamsGaussMidLight_allCells[:,j])
    """  
    for j in midLightSigmoidComplete:
        [y0, yrange, x0, delta_x] = fitParamsSigmoidMidLight_allCells[:,j]
        y = y0 + yrange/(1+np.exp((x0-amplitude)/delta_x)); 
        plt.plot(amplitude, response_midLight[:,j],'k')
        plt.plot(amplitude, y,'k--') 
        plt.show()
        print(j, miMidLight_allCells[j], rSquaredSigmoidMidLight_allCells[j], fitParamsSigmoidMidLight_allCells[:,j])
    ...
    highLightGaussLowRSquared.append(highLightGaussComplete)
    highLightGaussLowRSquaredFitParams.append(fitParamsGaussHighLight_allCells[:,highLightGaussComplete])
    highLightGaussLowRSquaredValues.append(rSquaredGaussHighLight_allCells[highLightGaussComplete])  
    ...
    for j in GaussianToKeep:        
        ...
        plt.plot(amplitude, y,'k--') 
        plt.show()
        print(j, miHighLight_allCells[j], rSquaredGaussHighLight_allCells[j], fitParamsGaussHighLight_allCells[:,j])
    """
    for j in highLightSigmoidComplete:
        [y0, yrange, x0, delta_x] = fitParamsSigmoidHighLight_allCells[:,j]
        y = y0 + yrange/(1+np.exp((x0-amplitude)/delta_x)); 
        plt.plot(amplitude, response_highLight[:,j],'k')
        plt.plot(amplitude, y,'k--') 
        plt.show()
        print(j, miHighLight_allCells[j], rSquaredSigmoidHighLight_allCells[j], fitParamsSigmoidHighLight_allCells[:,j])

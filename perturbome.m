...
load('MT068_20190903_FRA_OptoStim_03.mat');
load('MT068_20190903_RedCells_SlopeRemoval_Movie.mat');
load('F_MT068_20190903MT068_tifStacks_plane1_proc.mat');
...
Redcells_ROI = zeros(length(Ind_cells_red),2);
for i = 1:size(Redcells_ROI,1)
    Redcells_ROI(i,:) = mean(spatialInfo.ROIs{Ind_cells_red(i)});
end

DistbetRedcells = zeros(length(Ind_cells_red),length(Ind_cells_red));
...
closestCells = cell(length(Ind_cells_red),1);
useMinDist = 1;
for i = 1:size(Redcells_ROI,1)
...
for ii = 1:length(Ind_cells_red)
    h = patch(spatialInfo.ROIs{Ind_cells_red(ii)}(:,2),spatialInfo.ROIs{Ind_cells_red(ii)}(:,1),'y','facealpha',0.8,'edgealpha',0,'LineWidth',1,'facecolor',[1 0.6 0.6]);
end
...
%% check which redcells respond to light

responsiveRedcells = [];
for i = 1:length(Ind_cells_red)
    responsiveRedcells = [responsiveRedcells, Ind_cells_red(i)*any(abs(Cells_ActivityChange2(:,Ind_cells_red(i))))];
end
...
highResponseRedcell = highResponseRedcell/0.7;
midResponseRedcell = midResponseRedcell/0.304;

fsig = figure();
histogram(signalCorrelationsRedcell);
title('MT068 20190903, signal correlations');
savefig(fsig, 'MT068_20190903_signalCorrelations_av_raster_poststim.fig')
...
title('MT068 20190903, high response');
savefig(fhigh, 'MT068_20190903_highResponse_av_raster_poststim.fig')
...
title('MT068 20190903, mid response');
savefig(fmid,'MT068_20190903_midResponse_av_raster_poststim.fig')

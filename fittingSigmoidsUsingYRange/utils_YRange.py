...
def sigmoid_fn(params,response,sem):
    ...
    Error is based on deviance.
    """
    y0, yrange, x0, delta_x = params[0], params[1], params[2], params[3]
...
def check_soundPositive(parameters, ifGauss):
    ...
    either yrange>0 and positive slope or yrange<0 and negative slope
    """
    if ifGauss:
        ifSoundPositive = parameters[0] > 0
    else:
        ifSoundPositive = (parameters[3] > 0 and parameters[1] > 0) or (parameters[3] < 0 and 0 > parameters[1])
        if ifSoundPositive and parameters[3]<0:
            y0 = np.copy(parameters[0])
            ymax = np.copy(parameters[1] + parameters[0])
            parameters[0] = ymax
            parameters[1] = y0
